...
import static com.zeoflow.storage.MasterKey.KEYSTORE_PATH_URI;
 ...
 * </pre>
 */
final class EncryptedSharedPreferences implements SharedPreferences
{

    private static final String KEY_KEYSET_ALIAS = ZeoFlowApp.getContext().getPackageName() + ".prefs.key.keyset";
    private static final String VALUE_KEYSET_ALIAS = ZeoFlowApp.getContext().getPackageName() + ".prefs.value.keyset";
            ...
            throws GeneralSecurityException, IOException
    {
        ...
        final Context applicationContext = ZeoFlowApp.getContext();
    ...
    private static final class Editor implements SharedPreferences.Editor
    {
                                                     ...
                                                     @Nullable Set<String> values)
        {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N)
            {
                totalBytes = values.size() * Integer.BYTES;
            }
        ...
        public SharedPreferences.Editor putFloat(@Nullable String key, float value)
        {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N)
            {
                buffer = ByteBuffer.allocate(Integer.BYTES + Float.BYTES);
            } else
            {
                buffer = ByteBuffer.allocate(10);
            }
